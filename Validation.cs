...
namespace Collection.Latihan1
{
    class Validation
    {
        ...
        public static string ReadInputStringOption(string pertanyaan, bool mandatory, string option1, string option2)
        {
            bool valid = false;
            string output;
            ...
            do
            {
                ...
                if (!string.IsNullOrEmpty(output))
                {
                    if (output.ToUpper() != option1 && output.ToUpper() != option2)
                    {
                        Console.WriteLine("Inputan harus [{0}]", (option1 + "/" + option2));
                    }
                    else
                    {
                        valid = true;
